...
- Hlavičkové komentáře metod a vlastností piš vždy jako platnou XML dokumentaci C# s `///` a odpovídajícími XML značkami, například `<summary>`, podle potřeby také `<param>`, `<returns>` nebo `<value>`. Nikdy je nezjednodušuj na obyčejné komentáře s `//`. V celém upravovaném souboru převeď i historické hlavičkové komentáře do tohoto formátu a zachovej jejich význam. Pravidlo určuje formát hlavičkové dokumentace; běžné komentáře uvnitř implementace mohou nadále používat `//`.
